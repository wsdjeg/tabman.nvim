# tabman.nvim - Neovim Plugin Assistant

A lightweight tab and window manager for Neovim. This plugin provides an
interactive view to navigate, inspect, and manage tabpages and windows.
...
- **Verify before writing**: Only reference commands that actually exist in the codebase. Check `lua/tabman.lua` for the `open()` API and `plugin/tabman.lua` for the `:Tabman` command.
- **No invented commands**: Do not create commands that don't exist in the code.
...
tabman.nvim/
├── lua/
│   ├── tabman.lua              # Main module - tab/window manager
│   └── picker/
│       └── sources/
│           └── tabman.lua      # picker.nvim source
├── plugin/
│   └── tabman.lua              # Plugin entry (:Tabman command)
├── syntax/
│   └── tabman.lua              # Syntax file for tabman buffer
├── test/
│   ├── minimal_init.lua        # Headless test config
│   ├── run.lua                 # Test runner
│   ├── install_deps.lua        # Cross-platform dependency installer
│   ├── example_spec.lua        # Example test
│   └── .deps/                  # Test dependencies (luaunit)
├── .github/
│   ├── workflows/
...
- **Lua** - Use 2-space indentation, snake_case for local variables, camelCase for functions
- **Module pattern** - Use `local M = {}` and `return M` pattern
- **Type annotations** - Use `---@class`, `---@param`, `---@return` EmmyLua annotations
- **No globals** - All functions should be local or module-level
- **Error handling** - Use `pcall` for API calls that may fail
